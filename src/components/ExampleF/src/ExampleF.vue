...
const init = (): void => {
  ...
  // scene.fog = new Fog(0xaaaaaa, 0.015, 100);
  scene.fog = new FogExp2(0xaaaaaa, 0.015);
  // 坐标系
  ...
  const controls = {
    ...
    addCube: function () {
      const cubeGeometry = new BoxGeometry(4, 4, 4);
      const cubeMaterial = new MeshLambertMaterial({ color: `#${Math.floor(Math.random() * 16777215).toString(16)}` });
      const cube = new Mesh(cubeGeometry, cubeMaterial);
      cube.castShadow = true;
